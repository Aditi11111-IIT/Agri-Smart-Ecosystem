...
def create_pdf(user, soil_name, scores):
    ...
    pdf.cell(200, 10, txt="Crop Suitability Analysis:", ln=True)
    pdf.set_font("Arial", size=12)
    for crop, score in scores.items():
        pdf.cell(200, 10, txt=f"- {crop}: {round(score*100, 2)}% Match", ln=True)
    pdf.ln(20)
    pdf.set_font("Arial", 'I', 10)
    pdf.cell(200, 10, txt="Generated by Agri-Smart Ecosystem (A.S.E) - Digital India Initiative", ln=True, align='C')
    return pdf.output(dest='S').encode('latin-1')

def calculate_suitability(vec):
    """Vector Mathematics for Crop Prediction"""
    crops_db = {
...
if not st.session_state.logged_in:
    ...
    if w_data and w_data.get("main"):
        ...
        st.warning("Weather API currently unavailable. Check your API key or City name.")
    ...
    if 'soil_vec' in st.session_state:
        ...
        with c_an:
            ...
            st.download_button(label="📥 Download Soil Report PDF", 
                               data=pdf_bytes, 
                               file_name=f"Soil_Report_{st.session_state.user}.pdf",
        ...
        with c_kn:
            ...
            if os.path.exists("data/schemes.csv"):
                ...
                st.error("Schemes data file missing.")
